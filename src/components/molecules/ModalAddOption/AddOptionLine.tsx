...
export const AddOptionLine: React.FC<Props> = ({ option, onClickAdd }) => {
  ...
  return (
    <div key={option.name} className={styles.AddOptionLine}>
      <Button className={styles.AddOptionLineButton} onClick={() => onClickAdd(withSubOption)}>
        {option.name}
      </Button>
      ...
      {option.subOption && (
        <>
          <input type="checkbox" onChange={() => setWithSubOption(!withSubOption)} />
